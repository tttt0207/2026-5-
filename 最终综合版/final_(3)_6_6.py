...
def classify_quad(pts, contour=None):
    ...
    说明：
    1. 先判断正方形，正方形必须同时满足四边接近、对角线接近、角度接近 90 度；
    2. 只要上下底差异明显，就优先进入梯形；
    3. 梯形再按角度细分：
       - 有连续两个直角：right_trapezoid
       - 两条腰长度接近：isosceles_trapezoid
       - 兜底：trapezoid
    """
    ...
    # 四个角的 90 度接近程度，越小越接近 90 度
    cos_tl = angle_cos(bl, tl, tr)
    cos_tr = angle_cos(tl, tr, br)
    cos_br = angle_cos(tr, br, bl)
    cos_bl = angle_cos(br, bl, tl)
    max_cos = max(cos_tl, cos_tr, cos_br, cos_bl)
    ...
    # 正方形条件收紧：
    # 必须上下底接近、左右腰接近，否则很容易把等腰梯形误判成正方形。
    is_square = (
        side_ratio > 0.78 and
        diag_ratio > 0.78 and
        max_cos < 0.55 and
    ...
    # 梯形细分：
    # 直角梯形通常会有同一条腰两端都是直角。
    # cos < 0.30 大约表示角度在 72.5°~107.5° 之间，可以根据实测再收紧到 0.25。
    right_angle_tl = cos_tl < 0.30
    right_angle_tr = cos_tr < 0.30
    right_angle_br = cos_br < 0.30
    right_angle_bl = cos_bl < 0.30

    left_leg_right = right_angle_tl and right_angle_bl
    right_leg_right = right_angle_tr and right_angle_br

    if left_leg_right or right_leg_right:
        base_extra["trapezoid_type"] = "right"
        base_extra["right_angle_side"] = "left" if left_leg_right else "right"
        return "right_trapezoid", base_extra

    # 等腰梯形：两条腰长度接近。
    # 你说比赛中只会有直角梯形或等腰梯形，所以不满足直角时优先按等腰处理。
    if left_right_diff < 0.25:
        base_extra["trapezoid_type"] = "isosceles"
        base_extra["right_angle_side"] = "none"
        return "isosceles_trapezoid", base_extra

    # 兜底：仍然归到普通梯形，避免识别失败。
    base_extra["trapezoid_type"] = "unknown"
    base_extra["right_angle_side"] = "none"
    return "trapezoid", base_extra
...
def detect_shape_in_warped(warped_a4):
    ...
    # 四边形：正方形 or 梯形
        ...
        if shape_name in ("trapezoid", "isosceles_trapezoid", "right_trapezoid"):
            shape_info["trapezoid_type"] = extra.get("trapezoid_type", "unknown")
            shape_info["right_angle_side"] = extra.get("right_angle_side", "none")
    ...
    elif shape_info["shape"] in ("trapezoid", "isosceles_trapezoid", "right_trapezoid"):
...
# 多边形拐点停顿时间，单位秒。
# 作用：正方形、三角形、梯形/直角梯形到达每个拐角后先停一下，
# 避免刚到拐点就立刻切换到下一条边，导致云台误差方向突变、拐角被拉歪。
# 建议先从 0.15~0.30 调试；越大拐点越稳，但 UV 纸拐角处停留痕迹会更明显。
CORNER_PAUSE_SEC = 0.20
...
def polyline_points_cm_with_corners(vertices, closed=True, step_cm=LINE_STEP_CM):
    """
    生成折线轨迹，同时返回需要停顿的拐点下标。

    返回：
    points：完整离散轨迹点
    corner_indices：到达这些下标时需要停顿。默认不包含起点和最终闭合点。
    """
    if not vertices:
        return [], set()
    ...
    for i in range(end):
        ...
        seg_points = interpolate_line_cm(p1, p2, step_cm)
        points.extend(seg_points)

        # 每一段的终点就是一个顶点。
        # i == end - 1 时是闭合回起点，不需要停顿，后面会关激光。
        # 第一项 points[0] 是起点，起点已经由 START_STABLE_TIME_SEC 单独稳定等待。
        end_index = len(points) - 1
        if i < end - 1:
            corner_indices.add(end_index)
...
def build_fixed_task_path(state_code):
    """
    基础题轨迹 + 拐点停顿下标。
    返回 points, corner_indices。
    """
    if state_code == 0x12:
        vertices = [(-3.0, 3.0), (3.0, 3.0), (3.0, -3.0), (-3.0, -3.0)]
        return polyline_points_cm_with_corners(vertices, closed=True)
...
def build_path_from_shape_info_for_draw(shape_info):
    """
    发挥题：根据识别结果生成轨迹，并返回多边形拐点停顿下标。
    圆形不设置拐点停顿。
    """
    if shape_info is None:
        return [], set()
    ...
    if shape in ("trapezoid", "isosceles_trapezoid", "right_trapezoid"):
        top = float(shape_info.get("top_cm", 12.0))
        bottom = float(shape_info.get("bottom_cm", 16.0))
        height = float(shape_info.get("height_cm", 10.0))

        # 保证 bottom 是长底，top 是短底。若识别结果相反，自动交换。
        if top > bottom:
            top, bottom = bottom, top

        if shape == "right_trapezoid":
            right_angle_side = shape_info.get("right_angle_side", "left")

            if right_angle_side == "right":
                # 右侧为直角边：右上点和右下点 x 坐标对齐
                vertices = [
                    (bottom / 2.0 - top, height / 2.0),
                    (bottom / 2.0, height / 2.0),
                    (bottom / 2.0, -height / 2.0),
                    (-bottom / 2.0, -height / 2.0),
                ]
            else:
                # 左侧为直角边：左上点和左下点 x 坐标对齐
                vertices = [
                    (-bottom / 2.0, height / 2.0),
                    (-bottom / 2.0 + top, height / 2.0),
                    (bottom / 2.0, -height / 2.0),
...
def main():
    ...
    task_state = 0x00
    target_points = []
    corner_pause_indices = set()
    current_index = 0
    corner_pause_index = -1
    corner_pause_start = 0.0
                ...
                if sys_state == SYS_IDLE:
                    ...
                    if cmd_state is not None:
                        ...
                        if task_state in (0x12, 0x13, 0x14):
                            target_points, corner_pause_indices = build_fixed_task_path(task_state)
            ...
            if sys_state == SYS_RECOGNIZE_TARGET:
                if warped_a4 is not None:
                    ...
                    valid_shape = (
                        ...
                        shape_info.get("shape") in ("square", "triangle", "trapezoid", "isosceles_trapezoid", "right_trapezoid", "circle")
                    )
                    ...
                    if valid_shape:
                        ...
                        if task_state == 0x25:
                            ...
                            if target_center_stable < TARGET_CENTER_STABLE_FRAMES:
                                ...
                                new_target_points, new_corner_pause_indices = build_path_from_shape_info_for_draw(shape_info)

                                if len(new_target_points) > 0:
                                    target_points = new_target_points
                                    corner_pause_indices = new_corner_pause_indices
                            ...
                            target_points, corner_pause_indices = build_path_from_shape_info_for_draw(shape_info)
            ...
            # 正式画图：激光已打开，发送 01...05
                ...
                if target_points and laser_cm is not None and current_index < len(target_points):
                    ...
                    if err <= ARRIVE_DISTANCE_CM and (now_switch - last_switch_time >= 0.05):
                        ...
                        # 多边形拐点停顿：
                        # 第一次到达该拐点时，只保持当前目标点不变，让 STM32 继续收到接近 0 的误差；
                        # 停顿时间到达后，才切换到下一段轨迹。
                        if current_index in corner_pause_indices:
                            if corner_pause_index != current_index:
                                corner_pause_index = current_index
                                corner_pause_start = now_switch
                                print("到达拐点 idx=%d，停顿 %.2fs" % (current_index, CORNER_PAUSE_SEC))

                            if now_switch - corner_pause_start < CORNER_PAUSE_SEC:
                                pass
                            else:
                                corner_pause_indices.discard(current_index)
                                corner_pause_index = -1
                                corner_pause_start = 0.0
                                current_index += 1
