...
def classify_quad(pts, contour=None):
    ...
    改进点：
    1. 不只看四条边是否相等；
    2. 增加 minAreaRect 判断，纸张倾斜或镜头畸变时更稳；
    3. 不让轻微变形的正方形轻易掉到 trapezoid。
    """
    ...
    # 正方形条件：放宽
    # 纸张倾斜时四边可能不完全相等，所以增加 diag_ratio 和 rect_ratio 辅助判断
    is_square_by_side = side_ratio > 0.55 and max_cos < 0.75
    is_square_by_diag = diag_ratio > 0.62 and max_cos < 0.78
    is_square_by_rect = rect_ratio > 0.65 and top_bottom_diff < 0.45

    # 强兜底：如果最小外接矩形接近正方形，优先认为是 square
    if rect_ratio > 0.70 and side_ratio > 0.50:
    ...
    if is_square_by_side or is_square_by_diag or is_square_by_rect:
        side_px = sum(sides) / 4.0
...
def is_reasonable_square(pts, contour=None):
    ...
    if side_ratio > 0.60 and max_cos < 0.70:
        return True

    if diag_ratio > 0.68 and max_cos < 0.75:
        return True
