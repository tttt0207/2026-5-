...
注意：
- 这份代码默认不写串口，只在终端打印。
- 后续需要发给 32 时，可以在 print_control_info() 里调用 serial_send_packet()。
"""
...
def build_packet(dx_cm, dy_cm, target_index, flag):
    """
    串口帧格式建议：
    AA  dx_int16  dy_int16  target_id_uint8  flag_uint8  checksum  0A
    dx/dy 单位：0.1cm。
    checksum：从 AA 到 flag 的低 8 位累加和。
    """
    dx_i = int(round(dx_cm * 10))
    dy_i = int(round(dy_cm * 10))
    dx_i = max(-32768, min(32767, dx_i))
    dy_i = max(-32768, min(32767, dy_i))
    target_id = target_index & 0xFF
    flag_u8 = flag & 0xFF

    body = struct.pack("<BhhBB", 0xAA, dx_i, dy_i, target_id, flag_u8)
    checksum = sum(body) & 0xFF
    return body + struct.pack("<BB", checksum, 0x0A)
...
def main():
    ...
    try:
        while True:
            ...
            # 构造将来给 32 的数据包。dx/dy 为空时发 0。
            if dx is None or dy is None:
                packet = build_packet(0.0, 0.0, current_index, flag)
            else:
                packet = build_packet(dx, dy, current_index, flag)
